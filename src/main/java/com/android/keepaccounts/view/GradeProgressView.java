package com.android.keepaccounts.view;
...
import com.android.keepaccounts.R;


/**
 * MainActivity中显示百分比圆环的自定义View
 */
public class GradeProgressView extends View {
    ...
    private static final int DEFAULT_PROGRESS_COLOR = R.color.colorBrown;
    private static final int DEFAULT_BACKGROUND_COLOR = R.color.colorBrownLight;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        float degree = 3.6f * mProgress;

        //draw out arc
        canvas.drawArc(mOuterRectF, 0, 360, false, mOuterPaint);

        //draw background arc
        canvas.drawArc(mRectF, 0 , 360, false, mPaint);

        //draw progress arc
        canvas.drawArc(mRectF, 270, degree, false, mProgressPaint);

        //draw pointer
        //canvas.drawCircle(centerX, centerY, pointRadius, mInnerCirclePaint);

        canvas.save();
        //canvas.rotate(0 + degree, centerX, centerY);
